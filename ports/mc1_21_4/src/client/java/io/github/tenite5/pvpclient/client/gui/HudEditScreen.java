...
public class HudEditScreen extends Screen {
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float a) {
		graphics.drawCenteredString(this.font, "Drag overlays to move them", this.width / 2, 8, 0xFFFFD33D);
		graphics.drawCenteredString(this.font, "Drag the ball in the corner to resize - ESC when done", this.width / 2, 20, 0xFF8B8B9E);
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
			...
			for (int i = modules.size() - 1; i >= 0; i--) {
				...
				if (Math.abs(mouseX - ballCenterX) <= BALL && Math.abs(mouseY - ballCenterY) <= BALL) {
	...
	public boolean mouseDragged(double mouseX, double mouseY, int button, double dx, double dy) {
		if (this.resizingModule != null) {
			...
			int x = this.moduleX(module);
			int y = this.moduleY(module);
			float scaleX = (float) ((mouseX - x) / Math.max(1, module.contentWidth(this.minecraft)));
			float scaleY = (float) ((mouseY - y) / Math.max(1, module.contentHeight(this.minecraft)));
		...
		if (this.draggingModule != null) {
			...
			int w = module.scaledWidth(this.minecraft);
			int h = module.scaledHeight(this.minecraft);
			double x = Math.clamp(mouseX - this.grabOffsetX, 0, Math.max(0, this.width - w));
			double y = Math.clamp(mouseY - this.grabOffsetY, 0, Math.max(0, this.height - h));
