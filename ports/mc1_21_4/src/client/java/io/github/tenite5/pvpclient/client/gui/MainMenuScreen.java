...
public class MainMenuScreen extends Screen {
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float a) {
		...
		graphics.pose().translate(this.width / 2.0F, 12.0F, 0.0F);
		graphics.pose().scale(2.0F, 2.0F, 1.0F);
		graphics.drawCenteredString(this.font, "PVP CLIENT", 0, 0, 0xFFFFD33D);
		graphics.pose().popPose();
		graphics.drawCenteredString(this.font, "the pvp client for minecraft " + net.minecraft.SharedConstants.getCurrentVersion().getName(), this.width / 2, 30, 0xFF8B8B9E);
	...
	private void renderCard(GuiGraphics graphics, Module module, int index, int mouseX, int mouseY) {
		...
		// Name
		graphics.drawCenteredString(this.font, module.name(), x + CARD_W / 2, y + 45, 0xFFFFFFFF);
		...
		graphics.drawCenteredString(this.font, enabled ? "Deactivate" : "Activate", x + CARD_W / 2, by + 4, 0xFFFFFFFF);
		...
		graphics.drawCenteredString(this.font, "Like", bx + half / 2, ly + 4, 0xFFFFFFFF);
		graphics.drawCenteredString(this.font, "Edit", bx + half + (bw - half) / 2, ly + 4, 0xFFFFFFFF);
	}
	...
	private void renderBottomBar(GuiGraphics graphics, int mouseX, int mouseY) {
		...
		graphics.drawCenteredString(this.font, "Edit HUD", hx + 45, y + 5, 0xFF39C5CF);
		...
		graphics.drawCenteredString(this.font, label, kx + 65, y + 5, this.listeningForMenuKey ? 0xFFFFD33D : 0xFF9E6BFF);
	}

	@Override
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		if (this.listeningForMenuKey) {
			ConfigManager.menuKey = KeySetting.fromMouseButton(button);
			this.listeningForMenuKey = false;
			this.playClick();
			return true;
		}
		if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
			if (mouseY >= this.gridY() - 2 && mouseY < this.gridBottom()) {
				List<Module> modules = ModuleManager.all();
				for (int i = 0; i < modules.size(); i++) {
					...
					int bw = CARD_W - 12;
					int half = bw / 2;
					if (inside(mouseX, mouseY, bx, y + 58, bw, 16)) {
						module.toggle();
						this.playClick();
						return true;
					}
					if (inside(mouseX, mouseY, bx, y + 78, half, 16)) {
						module.setLiked(!module.isLiked());
						this.playClick();
						return true;
					}
					if (inside(mouseX, mouseY, bx + half, y + 78, bw - half, 16)) {
						this.playClick();
						this.minecraft.setScreen(new ModuleSettingsScreen(module, this));
						return true;
					}
				}
			}
			int by = this.bottomBarY();
			if (inside(mouseX, mouseY, this.hudButtonX(), by, 90, 18)) {
				this.playClick();
				this.minecraft.setScreen(new HudEditScreen());
				return true;
			}
			if (inside(mouseX, mouseY, this.keyButtonX(), by, 130, 18)) {
				this.playClick();
	...
	public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
		if (this.listeningForMenuKey) {
			ConfigManager.menuKey = keyCode == GLFW.GLFW_KEY_ESCAPE ? GLFW.GLFW_KEY_UNKNOWN : keyCode;
			this.listeningForMenuKey = false;
			return true;
		}
